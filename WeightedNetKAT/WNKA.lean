...
def 𝒪.aux (p₁ : RPol[F,N,𝒮]) (𝒪 : 𝕄[Pk[F,N], Pk[F,N], 𝕄[S p₁, 𝟙, 𝒮]]) :
    𝕄[Pk[F,N],Pk[F,N],𝒮] := (ι p₁ ⊠ 𝒪)∗
...
def 𝒪 (p : RPol[F,N,𝒮]) : 𝕄[Pk[F,N],Pk[F,N],𝕄[S p,𝟙,𝒮]] := fun α β ↦
  ...
  | wnk_rpol {~p₁*} =>
    R[(𝒪 p₁ <⊟ 𝒪.aux p₁ (𝒪 p₁)) α β,
      𝒪.aux p₁ (𝒪 p₁) α β]

def 𝒪_heart (p₁ : RPol[F,N,𝒮]) : 𝕄[Pk[F,N],Pk[F,N],𝒮] := (ι p₁ ⊠ 𝒪 p₁)∗

@[simp]
theorem 𝒪_iter_eq (p₁ : RPol[F,N,𝒮]) : 𝒪.aux p₁ (𝒪 p₁) = 𝒪_heart p₁ := by rfl

def δ.δ'' (p₁ : RPol[F,N,𝒮]) (δ : 𝕄[Pk[F,N],Pk[F,N],𝕄[S p₁,S p₁,𝒮]]) : 𝕄[Pk[F,N], Pk[F,N], 𝕄[S p₁, S p₁, 𝒮]] :=
  δ + (𝒪 p₁ ⊞ (𝒪_heart p₁ ⊟> (ι p₁ ⊡ δ)))
...
--     B[[δ' p₁ α β, 0],
--       [(𝒪_heart p₁ ⊟> (ι p₁ ⊡ δ p₁)) α β, 0]]
def δ.δ' (p₁ : RPol[F,N,𝒮]) : 𝕄[Pk[F,N], Pk[F,N], 𝕄[S p₁, S p₁, 𝒮]] := δ p₁ + (𝒪 p₁ ⊞ (𝒪_heart p₁ ⊟> (ι p₁ ⊡ δ p₁)))

@[simp] theorem δ.δ''_eq (p₁ : RPol[F,N,𝒮]) : δ.δ'' p₁ (δ p₁) = δ.δ' p₁ := rfl
...
theorem 𝒜_seq_eq {p₁ p₂ : RPol[F,N,𝒮]} : 𝒜⟦~p₁ ; ~p₂⟧ = (𝒜⟦~p₁⟧ ♢ 𝒜⟦~p₂⟧) := by
  ...
  · simp only [𝒜_def', ↓reduceIte, ι, 𝒪, Matrix.C_mul_R, Matrix.mul_sum, ← Matrix.mul_assoc,
    ...
    one_smul]
  · simp [𝒜_def'', ι, 𝒪, δ.eq_def (p:=p₁.Seq p₂), Matrix.C_mul_B, ← Matrix.mul_assoc, G.concat_apply, GS.splitAtJoined, xδ]
